...
class Generator(nn.Module):
    def __init__(self, patchsizes, ngf=32, max_ngf=256):
        super().__init__()
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        conv1 = self.conv1(x)
        #print ('disc1', conv1.shape)
        conv2 = self.conv2(conv1)
        #print ('disc2', conv2.shape)
        conv3 = self.conv3(conv2)
        #print ('disc3', conv3.shape)
        conv4 = self.conv4(conv3)
        #print ('disc4', conv4.shape)
        conv5 = self.conv5(conv4)
        #print ('disc5', conv5.shape)
...
class Encoder(nn.Module):
    def __init__(self, ngf=32):
        ...
        self.encoder4 = ResBlock(in_ch=ngf*4, out_ch=ngf*8, kernel_size=3, stride=2, padding=1)
        self.encoder42 = ResBlock(in_ch=ngf*8, out_ch=ngf*8, kernel_size=3, stride=1, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, ngf):
        super().__init__()
        self.decoder1 = ResBlock(in_ch=ngf*8, out_ch=ngf*4, kernel_size=3, stride=1, padding=1)
        self.decoder12 =ResBlock(in_ch=ngf*4, out_ch=ngf*4, kernel_size=3, stride=1, padding=1)
        
        self.decoder2 = ResBlock(in_ch=ngf*4, out_ch=ngf*2, kernel_size=3, stride=1, padding=1)
        self.decoder22 = ResBlock(in_ch=ngf*2, out_ch=ngf*2, kernel_size=3, stride=1, padding=1)
        
        self.decoder3 = ResBlock(in_ch=ngf*2, out_ch=ngf, kernel_size=3, stride=1, padding=1)
        self.decoder32 = ResBlock(in_ch=ngf, out_ch=ngf, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.decoder12(x)
        #print ('decoder12', x.shape)
        x = torch.nn.functional.interpolate(x, scale_factor=2, mode='bilinear',
                          align_corners=True)
        #print ('inter-2', x.shape)
        x = self.decoder2(x)
        #print ('decoder2', x.shape)
        x = self.decoder22(x)
        #print ('decoder22', x.shape)
...
class MultiAttn(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, dis=None):
        ...
        B,N,C = key.size()
        num_hidden_per_attn = C // self.h    #whats done here?
